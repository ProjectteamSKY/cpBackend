...
async def generate_access_token(code: str):
    ...
    print("TOKEN STATUS: - oauth_service.py:120", response.status_code)
    print("TOKEN RESPONSE: - oauth_service.py:121", response.text)
...
async def refresh_access_token():
    ...
    print("REFRESH STATUS: - oauth_service.py:192", response.status_code)
    print("REFRESH RESPONSE: - oauth_service.py:193", response.text)

    if response.status_code != 200:
        raise Exception(f"Refresh Error: {response.text}")

    if not response.text.strip():
        raise Exception("Empty response from refresh API")
